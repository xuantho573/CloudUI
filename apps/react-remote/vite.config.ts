...
export default defineConfig(async ({ mode }) => {
  ...
  const port = await reservePort(Number(env.REACT_REMOTE_PORT ?? 0));
  ...
  return {
    ...
    base: "./",
    plugins: [react(), publishRemote({ name: mfConfig.name }), federation(mfConfig)],
    server: {
      // publishRemote records this port so the host can find us; nothing
      // hardcodes it. Set REACT_REMOTE_PORT to pin it.
      port,
